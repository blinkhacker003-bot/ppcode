...
UHTPatch({
    ...
    apply: function() {
        ...
        var operatorPayloads = ['UHT_GAME_CONFIG_SRC["extend_events_listener"] = "window";'];
        var operatorIndex = operatorList.indexOf(UHT_GAME_CONFIG_SRC.integrationType);
        if (operatorIndex != -1) {
            if (operatorPayloads[operatorIndex] != null)
                eval(operatorPayloads[operatorIndex]);
            var path = "";
            ...
            path += UHT_GAME_CONFIG_SRC.integrationType + ".js";
...
UHTPatch({
    ...
    apply: function() {
        var featureCost = 0;
        if (window["FreeSpinsPurchaseManager"] != undefined) {
            var oFSPM_PFS = FreeSpinsPurchaseManager.prototype.PurchaseFreeSpins;
            FreeSpinsPurchaseManager.prototype.PurchaseFreeSpins = function(fspIndex) {
                oFSPM_PFS.apply(this, arguments);
                var fsPurchaseConfig = XT.GetObject(Vars.FreeSpinsPurchaseConfig);
                if (fsPurchaseConfig != null && fsPurchaseConfig.optionIndex > -1)
                    featureCost = fsPurchaseConfig.purchaseOptions[fsPurchaseConfig.optionIndex].bet * CoinManager.GetNextBet();
            };
        }
        if (window["FeaturePurchaseManager"] != undefined) {
            var oFPM_PF = FeaturePurchaseManager.prototype.PurchaseFeature;
            FeaturePurchaseManager.prototype.PurchaseFeature = function(fpIndex) {
                oFPM_PF.apply(this, arguments);
                var featurePurchaseData = XT.GetObject(Vars.FeaturePurchase);
                if (featurePurchaseData != null && featurePurchaseData.purchaseIndex > -1)
                    featureCost = featurePurchaseData.purchaseCosts[featurePurchaseData.purchaseIndex] * CoinManager.GetNextBet();
            };
        }
        ...
        NetPositionFOX.prototype.DeductBet = function( /**Object*/ dict) {
            ...
            if (coin != -1 && lines != -1 && dict[GameProtocolDictionary.FreeRound.TotalWin] == undefined) {
                if (featureCost > 0) {
                    this.currentNetPosition -= featureCost;
                    featureCost = 0;
                } else
